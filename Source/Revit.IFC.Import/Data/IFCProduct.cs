      ...
      protected override void Create(Document doc)
      {
         ...
         if (HasValidTopLevelGeometry() || IsHybridImportContainer())
         {
            ...
            using (IFCImportShapeEditScope shapeEditScope = IFCImportShapeEditScope.Create(doc, this))
            {
               ...
               {
                  // Had to make a small change here because my compiler didn't want to compile the original code, so I replaced
                  // it with some equivalent code that did compile.
