...
namespace BIM.IFC.Export.UI
{
   ...
   public class IFCCommandOverrideApplication : IExternalApplication
   {
      ...
      public static IList<IFCLinkExportSummary> PerformLinkedExportsOldVersion(Autodesk.Revit.DB.Document document, string folderpath, string fileName, Dictionary<ElementId, string> linksGUIDsCache, IFCExportOptions exportOptions)
      {
         // get the extension
         int index = fileName.LastIndexOf('.');
         if (index <= 0)
         ...
         foreach (String linkPathName in rvtLinkNamesToInstancesDict.Keys)
         {
            ...
            {
               exportSummary.pathDoesntExist.Add(linkPathName);
               continue;
            }
            ...
            {
               exportSummary.noTempDoc.Add(linkPathName);
               continue;
            }
      ...
      {
         IList<IFCLinkExportSummary> summaries = null;
         try {
            summaries = PerformLinkedExports(document, "", fileName, linksGUIDsCache, exportOptions);
         }
         catch { 
            summaries = PerformLinkedExportsOldVersion(document, "", fileName, linksGUIDsCache, exportOptions);
         }
