...
namespace BIM.IFC.Export.UI
{
   ...
   public class IFCCommandOverrideApplication : IExternalApplication
   {
      ...
      public static IFCExportFailuresContainer PerformLinkedExportsOldVersion(Autodesk.Revit.DB.Document document, string folderpath, string fileName, Dictionary<ElementId, string> linksGUIDsCache, IFCExportOptions exportOptions)
      {
         // get the extension
         int index = fileName.LastIndexOf('.');
         if (index <= 0)
         ...
         foreach (String linkPathName in rvtLinkNamesToInstancesDict.Keys)
         {
            ...
            {
               errContainer.pathDoesntExist.Add(linkPathName);
               continue;
            }
            ...
            {
               errContainer.noTempDoc.Add(linkPathName);
               continue;
            }
      ...
      {
         IFCExportFailuresContainer errContainer = null;
         try
         {
            // Newer version of export linked documents
            errContainer = PerformLinkedExports(document, "", fileName, linksGUIDsCache, exportOptions);
         }
         catch
         {
            // Older version which uses copy of linked file, doesn't work for cloud documents
            errContainer = PerformLinkedExportsOldVersion(document, "", fileName, linksGUIDsCache, exportOptions);
         }
      ...
      public static IFCExportFailuresContainer ExportLinkedDocuments(Autodesk.Revit.DB.Document document, string folderpath, string fileName, Dictionary<ElementId, string> linksGUIDsCache, IFCExportOptions exportOptions)
      {
         ...
         {
            // Older version which uses copy of linked file, doesn't work for cloud documents
            return PerformLinkedExportsOldVersion(document, folderpath, fileName, linksGUIDsCache, exportOptions);
         }
      }
